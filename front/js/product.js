...
function saveOrder(color, quantity) {                                               // Après avoir cliqué sur "Ajouter dans le panier" on va sauvegarder dans le localStorage les informations de l'article à acheter (l'id, la couleur, la quantité, le prix etc.) 
    const key = `${id}-${color}`                                                    // On crée une clé unique pour chaque article en combinant l'id du produit et la couleur sélectionnée. Cela permet de stocker plusieurs articles similaires mais avec des couleurs différentes dans le localStorage. Or cela était impossible avant sans "-${color}`". On utilise la syntaxe `${}` pour interpoler les valeurs de id et color dans la chaîne de caractères. l'id est récupéré dansla ligne 9 et la couleur est récupérée dans la ligne 95 (const color = document.querySelector("#colors").value)
    ...
    const itemToUpdate = InStorage.find((item) => item.id === id && item.color === color);                      // On vérifie si l'article existe déjà dans le panier (dans le localStorage) en utilisant la méthode find. On compare l'id et la couleur de l'article pour trouver une correspondance. Si l'article existe, on le met à jour.
